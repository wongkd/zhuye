...
- 线上版本：`v1.0` 已发布并继续小步修复。
- 最新提交：`ae76074 style: refine mobile layout with Apple-inspired spacing`。
- 1.0 已包含：Apple 风格桌面滚动揭示动画、编辑入口密码认证、编辑器图片上传裁剪压缩、顶部按钮精简、公司 LOGO 上传入口、透明图形 icon + 公司名称。
- 最新移动端优化：参考苹果官网手机页，将手机端改为浅灰/白色分区、居中首屏、胶囊按钮、全宽主视觉、数据后置到主视觉下方，提升排版比例和高级感。
