...
- 当前推进版本：`v1.7`，在 1.6 手机端稳定优化基础上补齐显式主题切换体验。
- 最新提交：以仓库 `main` 为准；1.7 本地改动完成验证后再提交。
...
- 1.7 新增：导航栏显式浅色 / 深色 / 系统主题切换，状态保存到 `localStorage: blry-theme-mode`，`theme-init.js` 首屏同步读取避免闪烁。
- 1.8 CMS 增强：编辑器读取 GitHub `content.js` 当前 SHA，保存时带 `baseSha` 做乐观锁防覆盖；新增“本次修改说明”，Worker 自动规范为 `cms: ...` 提交信息；历史列表优先展示可读内容标题。
